...
describe('TopicTextToolComponent', () => {
    ...
    const checkMath = (curText, replaceText) => {
        for (let i = 0; i < curText.length; i++) {
            ...
            if (curText[i] != replaceText[i]) {
                console.log('No match at ' + i + ' ' + curText[i] + ' vs ' + replaceText[i]);
            }
        }
    };
    beforeEach(async(() => {
    ...
    it('should check if has something selected', async(() => {
        const currentText = 'This text should be display with some other text at the end';
...
other Code`;
    ...
    it('should turn selected text to an image', async(() => {
        const link = 'https://rockcontent.com/es/wp-content/uploads/2019/02/google-trends-1280x720.png';
        const currentText = `
    this text starts
    ${link}
    this one ends
...
//# sourceMappingURL=topic-text-tool.component.spec.js.map